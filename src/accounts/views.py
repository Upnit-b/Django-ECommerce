...
def register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # creating user using custom create_user method
            ...
            # instead of sending the message, we can send the user to a new url display a message to on the login page about verifying the email
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            try:
                # check if the cart is created when the user is not logged in
                cart = Cart.objects.get(cart_id=_cart_id(request))
                cart_item_exists = CartItem.objects.filter(
                    cart=cart).exists()
                if cart_item_exists:
                    # this will get all the cart items that are in the cart
                    ...
                    # checking if the current variation is in existing variation list
